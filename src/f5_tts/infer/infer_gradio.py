...
with gr.Blocks() as app_tts:
    ...
    with gr.Accordion("Advanced Settings", open=False):
        ...
        remove_silence = gr.Checkbox(
            ...
            info="The model tends to produce silences, especially on longer audio. We can manually remove silences if needed. Note that this is experimental and may produce strange results. This will also increase generation time.",
            value=False,
        )
    ...
    def basic_tts(
        ...
        nfe_slider,
        speed_slider,
        normaliser_choice_input,  # A normaliser választása
    ):
        # -- Normaliser step --
        if normaliser_choice_input != "None":
            ...
            if os.path.exists(normaliser_file):
                ...
                if hasattr(normaliser_module, "normalize"):
                    ...
                    print("normaliser.py does not have a 'normalize' function, using original text.")
                    processed_text = gen_text_input
            else:
                print("normaliser.py not found for the chosen normaliser, using original text.")
                processed_text = gen_text_input
        else:
            processed_text = gen_text_input
        # ----------------------
...
with gr.Blocks() as app_multistyle:
    ...
    def add_speech_type_fn():
        ...
        if speech_type_count < max_speech_types:
            ...
            gr.Warning("Exhausted maximum number of speech types. Consider restarting the app.")
        return row_updates
    ...
    gen_text_input_multistyle = gr.Textbox(
        ...
        lines=10,
        placeholder="Use {StyleName} tokens to mark each style block in your text..."
    )
    ...
    def generate_multistyle_speech(
        ...
        *args,
        ...
        normaliser_choice -> args[3 * max_speech_types + 1]
        """
        speech_type_names_list = args[:max_speech_types]
        speech_type_audios_list = args[max_speech_types : 2 * max_speech_types]
        speech_type_ref_texts_list = args[2 * max_speech_types : 3 * max_speech_types]
        ...
        # Build dictionary of style -> (audio, ref_text)
        ...
        # Parse the text by style segments
        ...
        for segment in segments:
            ...
            try:
                ...
                gr.Warning(f"Please provide reference audio for style {current_style}.")
                return [None] + [speech_types[style]["ref_text"] for style in speech_types]

            ref_text = speech_types[current_style].get("ref_text", "")

            # Normalise the text if possible
            if normaliser_module and hasattr(normaliser_module, "normalize"):
            ...
            audio_out, _, ref_text_out = infer(
                ...
                show_info=print,  # minimal UI message
            )
        ...
        if generated_audio_segments:
            ...
            gr.Warning("No audio was generated.")
            return [None] + [speech_types[style]["ref_text"] for style in speech_types]
    ...
    with chat_interface_container:
            ...
            with gr.Column():
                with gr.Accordion("Advanced Settings", open=False):
                    ...
                    system_prompt_chat = gr.Textbox(
                        label="System Prompt",
                        value="You are not an AI assistant, you are whoever the user says you are. You must stay in character. Keep your responses concise.",
                        lines=2,
                    )
        ...
        conversation_state = gr.State(
            value=[
                {
                    "role": "system",
                    "content": "You are not an AI assistant, you are whoever the user says you are. You must stay in character. Keep your responses concise.",
                }
            ]
        )
        ...
        # Handle text input (Enter in textbox)
...
"""
    ...
    global_choose_normaliser = gr.Radio(
        choices=["None"] + normaliser_choices,
    ...
    # Integrate the existing sub-blocks
    gr.TabbedInterface(
    ...
    generate_btn.click(
        ...
        inputs=[
            ...
            nfe_slider,
            speed_slider,
            global_choose_normaliser,  # a normaliser komponens
        ],
        outputs=[audio_output, spectrogram_output, ref_text_input],
    )

    # 2) A Multi-Speech .click
    generate_multistyle_btn.click(
        fn=generate_multistyle_speech,
        inputs=[
        ...
        + [
            ...
            global_choose_normaliser,  # szintén a normaliser komponens
        ],
...
@click.option(
    ...
    default=None,
    type=str,
    help='The root path (or "mount point") of the application, if it\'s not served from the root ("/") of the domain. Often used when the application is behind a reverse proxy that forwards requests to the application, e.g. set "/myapp" or a full URL for application served at "https://example.com/myapp".',
)
@click.option(
    "--inbrowser",
    "-i",
...
if __name__ == "__main__":
    if not USING_SPACES:
        ...
        # In Spaces, just queue and launch
        app.queue().launch()
